...
def build_rank_html(title, result, weights_note="", generated=""):
    ...
    body = [
        ...
        '<div class="meta">Scenario Ranking - ', _html.escape(when), "</div></header>"
    ]

    # (1) Scoreboard
    body.append("<section><h2>Scenario Scoreboard</h2>")
    body.append("<table><tr><th>Rank</th><th>Scenario</th><th>Score</th><th style='width:50%'>Performance</th></tr>")
    ...
    if skipped_items:
        skipped_str = ", ".join(f"{_html.escape(item['key'])} ({_html.escape(item['reason'])})" for item in skipped_items)
    else:
        skipped_str = "none"
    body.append(f"<p style='font-size:11px; color:#7f8c8d; margin-top:12px;'>Skipped metrics: {skipped_str}</p>")
    body.append("</section>")
