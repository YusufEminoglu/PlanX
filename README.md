...
- **Plan dashboard & report** — the **Plan Dashboard & Performance Report (HTML)** Processing tool builds a one-click, single-file report with score cards and inline SVG charts — shareable with stakeholders, no separate dashboard dock required.
...
- **Scenario A/B comparison & ranking** — snapshot plan alternatives with the batchable Scenario Snapshot tool, diff two snapshots metric by metric, or rank any number of snapshots into a composite score with direction-aware, weighted min-max normalisation.
...
6. Open PlanX from its toolbar icon, then run **Plan Dashboard & Performance Report (HTML)** from the Studio panel or Processing for a shareable one-file report.
