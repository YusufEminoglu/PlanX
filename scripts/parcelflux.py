# -*- coding: utf-8 -*-
"""
ParcelFlux: Grid-Based Parcel Subdivision for PlanX

ParcelFlux divides input parcels into a grid of lots based on a specified lot width (in meters). It supports filtering by minimum and maximum parcel area, and can process only selected features if desired. The tool merges small polygons to avoid slivers and is designed for urban planning and land management workflows.

Default parameters:
- Minimum parcel area: 300 m²
- Maximum parcel area: 900 m²
- Selected only: False

The tool icon is 'parcelflux.png' in the plugin's icons directory.
"""
import processing
import math
...
class ParcelFluxAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.LOT_WIDTH,
            self.tr('Desired lot width (meters)'),
        ...
        self.addParameter(QgsProcessingParameterNumber(
            ...
            self.tr('Small parcel merge limit (% of average area)'),
            QgsProcessingParameterNumber.Double,
            25.0,
            minValue=0.0,
            maxValue=100.0
        ...
        self.addParameter(QgsProcessingParameterBoolean(
            ...
            self.tr('Distribute corners uniformly'),
            defaultValue=True
        ))
        self.addParameter(QgsProcessingParameterFeatureSink(
            self.OUTPUT,
            self.tr('Output divided parcels')
        ))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            ...
            if not features:
                raise QgsProcessingException(self.tr("No features found in the input layer."))

            feedback.pushInfo(f"Input layer loaded successfully. Feature count: {len(features)}")
            feedback.pushInfo(f"Lot width: {lot_width}")
            feedback.pushInfo(f"Min area: {min_area}")
            feedback.pushInfo(f"Max area: {max_area}")
            ...
            total = 100.0 / len(features) if features else 0
            has_irregular = False
            ...
            for feature in final_features:
                ...
                attrs = feature.attributes() + [round(area, 2)]
    ...
    def shortHelpString(self):
        return self.tr(
            """
            ParcelFlux subdivides input polygons into a regular grid of lots using the specified lot width (meters). You can restrict the output by minimum and maximum parcel area. Small polygons are merged to avoid slivers, and the output is added directly to the QGIS Layers panel.

            Parameters:
            - Input polygon layer: The polygon layer to subdivide.
            - Desired lot width (meters): The width of each grid cell (default: 16 m).
            - Minimum parcel area (m²): Exclude parcels smaller than this area (default: 300 m²).
            - Maximum parcel area (m²): Exclude parcels larger than this area (default: 900 m²).
            - Small parcel merge limit (% of average area): Small polygons below this percentage of average area are merged with neighbors (default: 25).
            - Distribute corners uniformly: If checked, corner lots are distributed more evenly.
            - Output divided parcels: The resulting subdivided parcels layer, with an 'aream2' field (area in m², rounded to 2 decimals).

            Usage notes:
            - Designed for urban and cadastral parcel subdivision.
            - Works best with rectangular or nearly rectangular parcels.
...
def run_tool():
    processing.execAlgorithmDialog(ParcelFluxAlgorithm())
