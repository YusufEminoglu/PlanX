...
class PlanPerformanceReportAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "covered-population share): treat it as a progress needle "
            "between iterations of the SAME plan, not as an absolute "
            "grade - a plan missing a section is scored only on what it "
            "provides.\n"
            "- Each card carries its own headline (mean access score, "
            "compliant categories, covered share): read the weakest card "
            "first, it names the workstream.\n"
            "- The histogram matters more than the mean: a two-peaked "
            "access distribution is two different cities in one plan.\n\n"
            "Using the results: regenerate the report after each planning "
            "iteration and file them - the PPI trace documents that the "
            "revisions actually improved the plan; use the standalone "
            "HTML for council/stakeholder distribution (no GIS needed to "
            "open it); for A/B comparison of two scenarios use Scenario "
            "Compare instead."
        )
