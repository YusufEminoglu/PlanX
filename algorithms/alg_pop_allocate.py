...
class PopAllocateAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- alloc_pop is a bookkeeping distribution, not a forecast: "
            "it places the increment EXACTLY (largest remainder - the "
            "parts always sum to the whole, unlike naive rounding) in "
            "proportion to the weights you chose. The result is only as "
            "meaningful as that weight: capacity-weighted = 'growth "
            "follows zoning', custom-weighted = whatever story the "
            "field encodes.\n"
            "- Zero-weight parcels get nothing by design - check that "
            "protected areas and full parcels really carry zero weight "
            "before reading the map.\n"
            "- Deterministic means reproducible: the same inputs give "
            "the same allocation, so scenarios differ only by their "
            "assumptions - as it should be.\n\n"
            "Using the results: add alloc_pop to current population for "
            "the horizon-year demand layer that Facility Adequacy, "
            "Green Access and the noise/air receivers consume - this is "
            "the standard bridge from a growth NUMBER to growth ON THE "
            "MAP; run one allocation per scenario weighting (compact "
            "infill vs greenfield) and compare the downstream metrics."
        )
