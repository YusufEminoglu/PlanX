...
class IsovistFieldAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- iso_area is perceived spaciousness; its GRADIENT is "
            "spatial drama - a narrow street opening onto a plaza is a "
            "sudden jump in iso_area, the sequence urban designers "
            "compose deliberately.\n"
            "- circular separates space types at a glance: high = "
            "rooms/plazas (gathering), low with high max_rad = "
            "corridors and vistas (movement). min_rad is intimacy - "
            "how close the nearest wall stands.\n"
            "- occlus distinguishes 'enclosed by buildings' from "
            "'limited by distance': high occlus = walls define the "
            "space (urban rooms); low occlus with big area = open "
            "field, no spatial definition at all.\n"
            "- Wayfinding reading: people orient along high-max_rad "
            "lines; a public space network whose isovists never "
            "overlap visually will feel disconnected even if the "
            "streets connect.\n\n"
            "Using the results: test a proposed square by rerunning "
            "with the new buildings - did it become a defined room "
            "(area up, occlus up, circular up) or a leftover gap?; "
            "check that landmark or gateway locations sit on "
            "high-visibility cells; low-iso_area + low-footfall alleys "
            "are the safety-audit shortlist."
        )
