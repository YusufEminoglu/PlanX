# -*- coding: utf-8 -*-
"""Land-Use Allocation Optimizer: assign parcels to uses by suitability."""
...
class LandUseAllocationAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            "Assigns a land use to each parcel to MAXIMISE total "
            "suitability while meeting a target area for each use - the "
            "spatial allocation problem at the heart of plan-making, solved "
            ...
            "not exceed its target and the area-weighted suitability is as "
            "high as possible. Parcels not needed to meet the targets are "
            "left unassigned; a use that cannot be filled reports a "
            "shortfall.\n\n"
            ...
            "a suitability field); those parcels consume that use's target.\n\n"
            "Method: greedy construction (best suitability first) plus a "
            "local search of reassignments and capacity-respecting swaps - "
            "a fast heuristic, not a guaranteed global optimum. Parcel area "
            "comes from the geometry unless an area field is given. Use a "
            "projected CRS for meaningful areas.\n\n"
            "Outputs:\n"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # map each suitability field (= a land use) to its target area
        ...
        feedback.pushInfo(self.tr(
            f"Allocating {len(feats)} parcels to {len(use_names)} use(s)..."))
        ...
        n_base = len(fields)
        suit_sum = np.zeros(len(use_names))   # area-weighted suitability per use
        ...
        mean_overall = res["objective"] / total_alloc if total_alloc > 0 else 0.0
        feedback.pushInfo(self.tr(
            f"Allocated {total_alloc:g} of {total_target:g} target area "
            f"({fill:.1f} percent); {unassigned_n} parcel(s) left over."))
        feedback.pushInfo(self.tr(
            f"Total suitability score {res['objective']:g} "
            f"(area-weighted mean {mean_overall:.3f}); "
            f"{res['reassigned']} reassignment(s), {res['swaps']} swap(s)."))
