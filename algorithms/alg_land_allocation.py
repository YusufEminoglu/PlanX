...
class LandUseAllocationAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- The map is a DRAFT ZONING PROPOSAL derived from your own "
            "suitability logic - read it as 'where the evidence wants "
            "each use', then edit for what the model cannot know "
            "(ownership, politics, phasing).\n"
            "- Summary first: a use with status 'Short' ran out of "
            "suitable land - either its target is too big for the study "
            "area or its suitability field is too strict. mean_suit per "
            "use shows the price of the pattern: if residential "
            "averages 0.9 and industry 0.4, industry is being pushed "
            "onto land the evidence dislikes.\n"
            "- Scattered single parcels of a use (with compactness 0) "
            "are honest suitability signals but poor zoning - raise the "
            "compactness weight or add adjacency rules and watch "
            "mean_suit: the drop measures what the tidier form costs.\n"
            "- Unassigned parcels are the residual land bank - the "
            "natural reserve list for future targets.\n\n"
            "Using the results: compare against the hand-drawn plan - "
            "parcels where they disagree deserve a written justification "
            "either way; test target scenarios (more housing, less "
            "industry) by editing the targets string and rerunning; use "
            "locks to freeze the parts already decided and let the tool "
            "fill the rest."
        )
