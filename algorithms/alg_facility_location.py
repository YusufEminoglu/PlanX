...
class FacilityLocationAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- rank and gain tell the investment story: rank 1 is the "
            "single best site, and each 'gain' is the extra demand that "
            "pick covers. When gain collapses (pick 4 adds a tenth of "
            "pick 1), that is your diminishing-returns point - the "
            "evidence-based answer to 'how many do we need?'.\n"
            "- reach_dem on UNSELECTED candidates shows near-misses: a "
            "candidate with high standalone reach that was not picked "
            "overlaps a winner - a plausible substitute if the winner's "
            "land falls through.\n"
            "- Coverage vs p-median give different answers by design: "
            "coverage chases the threshold (good for 'within 500 m' "
            "standards), p-median shortens everyone's average trip "
            "(good for services people use daily regardless). Run both; "
            "sites picked by BOTH are robust.\n\n"
            "Using the results: fix the sites you already own as "
            "'existing' and let the tool place only the increment; "
            "check the allocation output's uncovered points - if they "
            "cluster, no candidate serves that pocket and the candidate "
