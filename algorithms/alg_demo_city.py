...
class DemoCityAlgorithm(PlanXAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            ...
            QgsProcessingParameterNumber.Type.Integer, 4, minValue=1))
        self.addParameter(QgsProcessingParameterNumber(
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if streets_sink is not None:
            for idx, street in enumerate(res["streets"]):
                ...
                streets_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if buildings_sink is not None:
            for poly, height in res["buildings"]:
                ...
                buildings_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if landuse_sink is not None:
            for poly, use in res["landuse"]:
                ...
                landuse_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if pois_sink is not None:
            for (px, py), ptype in res["pois"]:
                ...
                pois_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if facilities_sink is not None:
            for (fx, fy), name, cap in res["facilities"]:
                ...
                facilities_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if demand_sink is not None:
            for (dx, dy), pop in res["demand"]:
                ...
                demand_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
        ...
        if green_sink is not None:
            for idx, poly in enumerate(res["green"]):
                ...
                green_sink.addFeature(feat, QgsFeatureSink.Flag.FastInsert)
