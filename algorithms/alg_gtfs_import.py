...
class GtfsImportAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- departures per stop is raw service PRESENCE: sort "
            "descending and the top stops are the network's true hubs, "
            "whatever the official hierarchy says. Stops with routes > 1 "
            "are the transfer skeleton.\n"
            "- The route table's service span exposes coverage in TIME: "
            "a route whose last departure is 19:00 does not serve "
            "evening shifts however dense its stops.\n"
            "- Trips per route separates paper routes (4 trips/day) "
            "from real service - a map of 'transit coverage' that "
            "ignores this flatters the network.\n\n"
            "Using the results: always eyeball the stops layer over a "
            "basemap first (agency coordinate bugs are common and will "
            "poison the downstream tools); check the chosen day - "
            "weekday vs weekend feeds can differ by half the service; "
            "then move to Transit Frequency Map for the rider's view "
            "and Transit Travel-Time Access for door-to-door times."
        )
