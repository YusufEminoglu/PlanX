...
class HeatRiskGridAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- The score is a relative screening index, not degrees: "
            "'Very High' cells are where surface cover argues hardest "
            "for overheating - dense, tall, unvegetated. Validate the "
            "top cells against night temperatures or complaints if "
            "available.\n"
            "- Read the component fractions to choose the REMEDY, not "
            "just the location: high built + low green = depave/plant; "
            "high height + low green = shade and albedo measures; "
            "moderate everything = check the neighbours (one cool cell "
            "in a hot district still overheats).\n"
            "- Because the scale is fixed, a scenario rerun is directly "
            "comparable: 'the park drops 14 cells out of Very High' is "
            "a legitimate sentence.\n\n"
            "Using the results: intersect Very High cells with vulnerable "
            "population (elderly, schools, clinics) to get priority "
            "cooling zones; test each greening candidate by rerunning "
            "and counting class changes; combine with Sky View Factor "
            "and Frontal Area Index to separate radiation-trap problems "
            "from ventilation problems."
        )
