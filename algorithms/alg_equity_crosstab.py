...
class EquityCrosstabAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- Go straight to class 1 (the worst-served quintile): any "
            "group with rep_ratio clearly above 1 there carries a "
            "disproportionate share of the deprivation. Above ~1.5 is "
            "hard to explain away; check the group's population first - "
            "tiny groups make noisy ratios.\n"
            "- The dissimilarity index sizes the segregation of a "
            "group's experience: 0.3+ means nearly a third of the group "
            "would have to 'move classes' to match everyone else - a "
            "structural pattern, not chance.\n"
            "- Compare group means AND P10s: equal means with unequal "
            "P10s = the group's worst-off are much worse off.\n"
            "- On the units output, map class 1 units of the "
            "over-represented group - equity findings only persuade "
            "when they become places on a map.\n\n"
            "Using the results: this is the table for an "
            "environmental-justice annex - 'group X is 1.8x "
            "over-represented in the worst access quintile' - and for "
            "testing remedies: rerun after the targeted investment and "
            "watch the class-1 rep_ratio move toward 1."
        )
