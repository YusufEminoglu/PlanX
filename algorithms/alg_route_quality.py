...
class RouteQualityAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- detour is the price of pleasantness: how much longer "
            "people must walk to avoid bad streets. Around 1.0 = the "
            "direct route is already fine; 1.3+ = the direct route is "
            "so poor it is worth a third more distance to escape it - "
            "a strong indictment of that corridor.\n"
            "- mean_score is the experienced quality of the trip (not "
            "of the city): school routes averaging under 50 are the "
            "'safe routes to school' work programme.\n"
            "- low_share > 0.3 with detour ~1.0 means there IS no "
            "better alternative - people are trapped on poor streets; "
            "that combination locates missing-link investments better "
            "than either number alone.\n\n"
            "Using the results: run home-to-school or home-to-station "
            "pairs and rank by low_share to prioritise footway and "
            "crossing upgrades; compare penalty 0 vs penalty 1 routes "
            "on a map - where they diverge is where quality, not "
            "distance, decides pedestrian behaviour; after an upgrade, "
            "the detour of nearby pairs falling toward 1.0 is the "
            "success metric."
        )
