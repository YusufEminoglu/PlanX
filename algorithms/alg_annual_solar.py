...
class AnnualSolarAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- The ratio cell / flat-ground-reference is the honest "
            "number: it isolates what the URBAN FORM does to solar "
            "access, independent of climate. > 0.9 = practically "
            "unobstructed roof; < 0.7 = the massing around it costs a "
            "third of the yield.\n"
            "- Rank roofs by the annual raster, then check the monthly "
            "bands of the shortlist: a roof that only performs "
            "April-September (low winter sun blocked by a neighbour) "
            "yields poorly exactly when demand and prices peak.\n"
            "- The peak-month statistic in the log doubles as a "
            "heat-exposure flag for open spaces.\n\n"
            "Using the results: multiply cell kWh/m2/yr by usable roof "
            "area and a system factor (~0.75-0.85, plus your climate's "
            "cloud correction) for a first PV estimate; write solar "
            "envelopes into the plan by testing candidate massings "
            "against the annual loss they impose on existing roofs; use "
            "the monthly bands to design seasonal shading (full summer "
            "shade, full winter sun) for squares and playgrounds."
        )
