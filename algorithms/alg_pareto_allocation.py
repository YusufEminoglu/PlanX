...
class ParetoAllocationAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- Plot suit_norm against compact_norm for the on-front rows: "
            "the SHAPE of the curve is the finding. A flat stretch means "
            "compactness is nearly free there (take it); a steep drop "
            "means further clustering starts costing real suitability.\n"
            "- The knee is the mathematically balanced pick, but it is a "
            "suggestion, not a verdict - a plan that values orderly "
            "zoning may sit right of the knee deliberately. The point is "
            "that you now know the price.\n"
            "- Dominated rows (on_front = 0) are still useful as "
            "sensitivity evidence: many dominated points close to the "
            "front = the solution is robust to the weight choice.\n\n"
            "Using the results: bring the front chart to the options "
            "discussion - 'scattered but optimal' vs 'compact but "
            "slightly worse' becomes a explicit, quantified choice; "
            "export the knee parcels as the working draft and hand-adjust "
            "from there; if the front collapses to one point, your "
            "suitability surfaces already agree with compact form - no "
