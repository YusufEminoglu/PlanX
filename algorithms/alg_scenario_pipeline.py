...
class ScenarioPipelineAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- The metric table scores the GROWN city, not today's: "
            "compare it against a snapshot of the present and the "
            "difference is what the growth pattern does to liveability "
            "- the usual finding is falling mean access, because new "
            "edge growth lands far from existing amenities. The SIZE "
            "of that drop is the planning information.\n"
            "- The chain is only as good as its inputs: the suitability "
            "raster decides where growth lands, and the amenity layers "
            "stay FIXED - so the result reads as 'growth without new "
            "services', the honest worst case for service planning.\n"
            "- Same seed = same scenario, so metric differences between "
            "runs are caused by your parameter changes, never by "
            "randomness.\n\n"
            "Using the results: run compact (high neighbourhood "
            "weight, constraints on) vs dispersed variants with the "
            "same demand and feed both JSONs to Scenario Compare - the "
            "access/walkability gap between them is the quantified "
            "cost of sprawl for YOUR city; where the grown scenario's "
            "access collapses, that is where the amenity investment "
            "belongs (test it with the standalone tools)."
        )
