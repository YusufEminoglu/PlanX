...
class CapacitatedSitingAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- This answers siting and sizing TOGETHER: a site is only "
            "picked if its capacity can actually absorb nearby demand, "
            "so the chosen set is a build programme, not just locations.\n"
            "- utilization near 1.0 on a selected site = the capacity "
            "you assumed is the binding constraint - if the real project "
            "can build bigger there, raise the capacity and rerun; the "
            "selection may simplify.\n"
            "- gain per rank is the business case per facility; when "
            "gain flattens, additional sites mostly shuffle demand "
            "rather than serve new people.\n"
            "- Uncovered output splits into 'too far from every open "
            "site' (a location gap - look for a missing candidate) and "
            "'in reach but everything full' (a capacity gap - size up).\n\n"
            "Using the results: run p = 1, 2, 3... and chart served "
            "demand vs p for the investment-staging curve; fix existing "
            "facilities so the tool only places the increment; sanity- "
            "check allocation lines - long lines crossing a competitor's "
            "catchment usually mean a barrier the network data missed."
        )
