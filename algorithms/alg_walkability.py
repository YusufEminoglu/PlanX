...
class WalkabilityAlgorithm(PlanXAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            "- walk_score bands: 70+ genuinely walkable; 50-70 walkable "
            "with friction; below 40 the street depends on cars. But "
            "the composite is only the headline - the SUB-SCORES name "
            "the deficiency, and each one maps to a different remedy:\n"
            "- low s_inters = coarse network (new links, cut-throughs); "
            "low s_mix = monofunctional zoning (permit corner uses); "
            "low s_dest = nothing to walk TO (ground-floor uses, "
            "kiosks, stops); low s_block = oversized blocks (mid-block "
            "crossings); low s_slope = topography (route around, "
            "handrails, stairs with ramps - not fixable by zoning).\n"
            "- A high-scoring segment next to very low ones reads as an "
            "island: walkability is a NETWORK property, so judge "
            "corridors, not single segments.\n\n"
            "Using the results: this layer is the standard 'existing "
            "conditions' exhibit of a pedestrian master plan - low-score "
            "corridors ranked by adjacent population are the investment "
            "list; rerun after the proposed interventions and difference "
            "the scores to show exactly which streets improve and by "
            "how much."
        )
